...
int main() {
    ...
    if (crash_data.get_crashes().get_size() == 0) {
        cerr << "No data loaded. Exiting.\n";
        return 1;
    }

    cout << "Total records loaded: " << crash_data.get_crashes().get_size() << endl;
    ...
    #pragma omp parallel sections
    {
        #pragma omp section
        {
            ...
            vector<int> brooklynCrashes = crash_data.searchByBorough("BROOKLYN");
        ...
        {
            start_time = chrono::high_resolution_clock::now();
            vector<int> severeCrashes = crash_data.searchByThreshold(3, 1);
            end_time = chrono::high_resolution_clock::now();
